...
export default function UploadPdfTemplatePage() {
  ...
  // 1. On mount, load distinct types from Firestore:
  useEffect(() => {
    (async () => {
      const snap = await getDocs(collection(db,'templates'));
      const distinct = new Set(snap.docs.map(d => d.data().templateType));
      // for any type not in BUILT_IN, add as docx type placeholder:
  ...
  // 2. When `type` changes, load existing:
  useEffect(() => {
    if (!type) {
      setExisting(null);
      setTitle(''); setRoles([]); setPreviewUrl(''); setFile(null);
      return;
    }
    (async () => {
      const q = query(
  ...
  const handleSubmit = async e => {
    e.preventDefault();
    // if "Add New", first register the new type in state
    if (type === '__add_new__') {
      // add to allTypes
      setAllTypes([
        ...allTypes,
    ...
    // normal submission:
    if (!type||!title||!file||roles.length===0) {
    ...
    setExisting({ id:docRef.id, name:title, templateType:type, roles, storagePath:path, url });
    setSaving(false);
  };

  // admin guard (replace with your real logic)
